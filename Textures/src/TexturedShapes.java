...
public class TexturedShapes {
    ...
    public static void Pyramid(GL2 gl2) {
		double r = 0.5d;
		int n = 15;
		float height = 1.0f;
	    ...
	    gl2.glTexCoord2f(xtexture, ytexture); // Texture coordinate for the top vertex
	    gl2.glVertex3fv(topPoint, 0);
		
		for(int i=0;i<=n;i++) {
			gl2.glNormal3fv(calculateNormal(topPoint, base[i], base[i+1]), 0);
			gl2.glTexCoord2f(xtexture + base[i][0], ytexture + base[i][2]); // Texture coordinates for the base vertices
		...
		gl2.glBegin(GL2.GL_TRIANGLE_FAN);
		gl2.glTexCoord2f(xtexture, ytexture); // Texture coordinate for the bottom vertex
	    gl2.glVertex3fv(botPoint,0);

		for (float[] i:base) {
			gl2.glNormal3f(0, -1, 0);
			gl2.glTexCoord2f(xtexture + i[0], ytexture + i[2]); // Texture coordinates for the base vertices
	        gl2.glVertex3fv(i,0);
		}
		
		gl2.glTexCoord2f(xtexture + base[0][0], ytexture + base[0][2]); // Texture coordinate for the first base vertex
