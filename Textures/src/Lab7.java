...
public class Lab7 extends JPanel implements GLEventListener {
	...
	private void triangularPrism(GL2 gl2) {
		...
		gl2.glTexCoord2d(0.5, 0);
		gl2.glVertex3d(-t,0.5,-0.25);
		gl2.glTexCoord2d(1, 0);
		gl2.glVertex3d(t,0.5,-0.25);
		gl2.glTexCoord2d(0.75, 1);
		...
		gl2.glTexCoord2d(0, 0.5);
		gl2.glVertex3d(-t,-0.5,-0.25);
		gl2.glTexCoord2d(0, 1);
		gl2.glVertex3d(-t,0.5,-0.25);
		gl2.glTexCoord2d(1, 1);
		gl2.glVertex3d(t,0.5,-0.25);
		gl2.glTexCoord2d(1, 0.5);
		gl2.glVertex3d(t,-0.5,-0.25);
		gl2.glEnd();
		
		gl2.glBegin(GL2.GL_TRIANGLE_FAN); // front left face
		gl2.glNormal3d(-0.75,0,t);
		gl2.glTexCoord2d(0.25, 0);
		gl2.glVertex3d(-t,0.5,-0.25);
		gl2.glTexCoord2d(0.5, 0);
		gl2.glVertex3d(-t,-0.5,-0.25);
		gl2.glTexCoord2d(0.5, 1);
		gl2.glVertex3d(0,-0.5,0.5);
		gl2.glTexCoord2d(0.25, 1);
		gl2.glVertex3d(0,0.5,0.5);
		gl2.glEnd();
	...
	private Texture textureFromResource(String resourceName) throws IOException {
		...
		BufferedImage image = ImageIO.read(Objects.requireNonNull(textureURL));
		
		Texture texture;
		ImageUtil.flipImageVertically(image);
		
		//uzyskanie aktualnego kontekstu z wątku
		GLContext context = displayGL.getContext();
		boolean needsToBeReleased = false;
		if(!context.isCurrent()) {
			context.makeCurrent();
			needsToBeReleased = true;
		}
		
		GL2 gl2 = context.getGL().getGL2();	//gl2 aktualnego kontekstu
	...
	private Texture textureFromPainting() {
		Texture texture;
		BufferedImage image = paintPanel.copyOSC();	//skopiowanie obrazu z paintPanek
	...
	public JMenuBar getMenuBar() {
		...
		ActionListener textureListener = new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				case 3:
					try {
						currentTexture = textureFromResource("textures/earth.jpg");
				...
				case 4:
					try {
						currentTexture = textureFromResource("textures/brick.jpg");
					} catch (IOException e1) {
						// TODO Auto-generated catch block
						e1.printStackTrace();
					}
				...
				case 5:
					try {
						currentTexture = textureFromResource("textures/clouds.jpg");
					} catch (IOException e1) {
						// TODO Auto-generated catch block
						e1.printStackTrace();
					}
		...
		makeMenuItem(objectMenu, "Pyramid (n=15)", objectListener, 7);
